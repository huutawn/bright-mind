...
from app.helpers.paging import (
    PaginationParams,
    paginate,
    Page,
)  # Giả sử paginate đã được sửa
...
class UserService:
    ...
    async def update_me(
        self, db: AsyncSession, user: User, data: UpdateUserReq
    ) -> UserResponse:
        ...
        user.logo = (
            user.logo if data.logo is None and user.logo is not None else data.logo
        )
        await db.commit()
    ...
    async def update(
        self, db: AsyncSession, user_id: int, data: UpdateUserReq
    ) -> UserResponse:
    ...
    async def get_all_user(self, db: AsyncSession, params: PaginationParams):
        ...
        _query = select(User).options(joinedload(User.user_profile))
        ...
        users = await paginate(
            db=db, model=User, query=_query, params=params, mapper=mapper
        )
